...
class EmbeddingModelTester:
    ...
    def test_model(self, model_name: str, model) -> Dict:
        ...
        try:
            embeddings = model.encode(content_list, show_progress_bar=True, batch_size=8)
        ...
        requires_bge_prefix = "bge-" in model_name and "large" not in model_name # bge-large doesn't need it
        ...
        for concept_name in ALL_VALIDATORS.keys():
            ...
            first_valid_pos = next(
                (i+1 for i, r in enumerate(valid_results) if r["valid"]), 
                None
            )
        ...
        overall_mrr = sum(r["reciprocal_rank"] for r in concept_results) / total_concepts
